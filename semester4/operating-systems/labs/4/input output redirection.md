...
2.  **Viewing the contents of tar files** Often you will see software distributed as a _gzipped tar file_. This is a traditional Unix style tape archive file (created with tar) that has been compressed with `gzip`. You can recognise these files by their traditional file extensions, ".tar.gz" or ".tgz". You can use the following command to view the directory of such a file on a Linux system:
```shell
